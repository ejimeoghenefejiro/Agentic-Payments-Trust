...
public sealed class EfConsumerPlanningStore:IConsumerPlanningStore
{
    ...
    public ConsumerPlanningConversation? FindLatestOpen(string principal,DateTimeOffset notBefore)=>_db.ConsumerPlanningConversations.AsNoTracking()
        .Where(x=>x.PrincipalId==principal&&(x.Status=="INVESTIGATING"||x.Status=="NEEDS_INPUT"||x.Status=="PROPOSE"))
        .AsEnumerable().Where(x=>x.UpdatedAt>=notBefore).OrderByDescending(x=>x.UpdatedAt).FirstOrDefault()is{} x?Map(x):null;
