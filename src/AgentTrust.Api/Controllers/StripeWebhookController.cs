...
public sealed class StripeWebhookController : ControllerBase
{
    ...
    public async Task<IActionResult> Receive(CancellationToken token)
    {
        ...
        {
            var attempt=await _db.ConsumerPaymentAttempts.SingleOrDefaultAsync(x=>x.ProviderPaymentId==providerPaymentId,token);
            var execution=await _db.PurchaseExecutions.SingleOrDefaultAsync(x=>x.ProviderPaymentId==providerPaymentId,token);
            ...
            if(execution is not null)
            {
                ...
                _audit.Append(new PurchaseAuditEvent($"pae_{Guid.NewGuid():N}", "StripeWebhookReconciled", execution.PurchaseIntentId,
